...
class AuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        try {
            ...
            {
                // keep the language the player signed up in
                if (\App\Support\Locales::normalize($request->header('X-Locale'))) {
                    $user->update(['language' => app()->getLocale()]);
                }
    ...
    public function submitForgetPassword(Request $request)
    {
        ...
        /// 5 characters was short enough to guess; the token is also tied to the
        /// e-mail address and expires (see submitResetPassword)
        $token = Str::random(40);
        ...
        try {
            ...
            \Log::error('Password reset e-mail failed: ' . $e->getMessage());
            DB::table('password_reset_tokens')->where('email', $request->email)->delete();

            return response()->json([
                'status'  => false,
                'error'   => __('We could not send the e-mail right now. Please contact support.'),
            ], 503);
        }

        return response()->json(['status' => true, 'message' => __('We sent a reset link to your e-mail!')], 200);
    }
    ...
    public function submitResetPassword(Request $request)
    {
        try {
            ...
            /// the token must belong to this e-mail and still be fresh; before, any
            /// valid token reset any account
            ...
            if(!empty($checkToken)) {
                ...
                if(!empty($user)) {
                    if($user->update(['password' => $request->password])) {
                        ...
                        return response()->json(['error' => __('Could not update the password')], 400);
                    }
                }else{
                    return response()->json(['error' => __('Email is not valid')], 400);
                }
            }

            return response()->json(['error' => __('Token is not valid')], 400);
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 400);
        }
    }
