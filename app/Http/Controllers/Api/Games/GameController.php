...
class GameController extends Controller
{
    ...
    public function sourceProvider(Request $request, $token, $action)
    {
        ...
        if (!in_array($action, $validEndpoints)) {
            return response()->json(['success' => false, 'message' => 'Invalid action'], 404);
        }

        // DecToken only returns status=true for a token signed by this server
        if(empty($tokenOpen['status']) || empty($tokenOpen['game']) || empty($tokenOpen['id'])) {
        ...
        if(empty($game) || $game->distribution !== 'source') {
        ...
        if(auth('api')->check()) {
            ...
            if(!empty($checkExist)) {
                if($checkExist->delete()) {
                    return response()->json(['status' => true, 'message' => __('Removed')]);
                }
            }else{
                ...
                if($gameFavoriteCreate) {
                    return response()->json(['status' => true, 'message' => __('Added')]);
                }
            }
        }
    }
    ...
    public function show(string $id)
    {
        ...
        if(!empty($game)) {
            if(auth('api')->check()) {
                $wallet = Wallet::where('user_id', auth('api')->id())->where('active', 1)->first()
                    ?? Wallet::where('user_id', auth('api')->id())->first();
                ...
                return response()->json(['error' => __('You need balance to play'), 'status' => false, 'action' => 'deposit' ], 200);
            }
            return response()->json(['error' => __('You need to log in to play'), 'status' => false ], 400);
        }
