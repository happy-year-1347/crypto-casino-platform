...
class Core
{
    ...
    public static function formatPixType($key)
    {
        switch ($key) {
            ...
            default:
                // crypto commission withdrawals store the coin code here
                return \App\Services\Crypto\NowPaymentsClient::DEFAULT_CURRENCIES[strtolower((string) $key)]['label'] ?? strtoupper((string) $key);
        }
    }
     ...
     * @return mixed
     */
    /** How long a game session token stays usable. */
    public const GAME_TOKEN_HOURS = 24;

    public static function MakeToken($array){
        if(is_array($array)){
            /// the game URL can end up in browser history or a screenshot, so the
            /// token carries an expiry that DecToken checks
            if(!array_key_exists('exp', $array)) {
                $array['exp'] = now()->addHours(self::GAME_TOKEN_HOURS)->timestamp;
            }
        ...
        /// The encoded payload is only obfuscated, not secret, so it is signed with the
        /// application key. DecToken refuses any token whose signature does not match,
        /// which stops a player from forging a token for another user id.
        $payload = self::urlSafe(self::Encode($output));
        return $payload . '.' . self::tokenSignature($payload);
    }
    ...
    public static function DecToken($token){
        ...
        if(count($parts) !== 2 || !hash_equals(self::tokenSignature($parts[0]), $parts[1])) {
            return array("status"=>false, "messase"=>"invalid token");
        }

        $json = self::Decode(self::urlSafeReverse($parts[0]));
        if(is_numeric($json)){
            return $token;
            ...
            /// expired game session: the player has to open the game again
            if(is_array($data) && !empty($data['exp']) && (int) $data['exp'] < time()) {
                return array("status"=>false, "messase"=>"expired token");
            }
    ...
    /**
     * HMAC signature for a game token payload, keyed with APP_KEY.
     *
     * @param string $payload
     * @return string
     */
    private static function tokenSignature(string $payload): string
    {
    ...
    /**
     * Tokens travel as a URL path segment (/api/vgames/{token}/spin), so the
     * base64 characters that would break routing are swapped for safe ones.
     */
    private static function urlSafe(string $value): string
    {
        return strtr($value, ['+' => '-', '/' => '_']);
    }

    private static function urlSafeReverse(string $value): string
    {
        return strtr($value, ['-' => '+', '_' => '/']);
    }
