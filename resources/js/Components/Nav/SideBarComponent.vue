    ...
    <aside :class="[
        ...
        <div class="h-full pb-4 overflow-y-auto sidebar-color p-4">
                ...
                <li v-if="!isAuthenticated" class="mb-4">
                    <button @click.prevent="$router.push('/register')"
                        ...
                        <div class="flex justify-center text-center self-center items-center">
                            ...
                            <!-- says the bonus that is actually configured, not a fixed claim -->
                            <span v-if="bonusPercent">{{ bonusPercent }}% {{ $t('Welcome Bonus') }}</span>
                            <span v-else>{{ $t('Welcome Bonus') }}</span>
                        </div>
                    </button>
                </li>
            </ul>
            ...
            <div class="grid grid-cols-2 gap-4 mt-4">
                <div>
                    <RouterLink :to="{ name: 'eventsPage' }" active-class="category-action" class="relative">
                        ...
                        <h1 class="button-title-menu">{{ $t('Events') }}</h1>
                    </RouterLink>
                </div>
                <div>
            ...
            <ul class="font-medium mt-5 mb-[200px]">
                <li class="px-3">
                    <a @click.prevent="$router.push('/support')" class="l-5 flex items-center w-full p-2 text-gray-700 font-normal transition duration-700 rounded-lg group dark:text-gray-400 dark:hover:text-white">
                ...
                <li class="px-3">
                    <a @click.prevent="$router.push('/support')" class="transition duration-700 bg-gray-100 dark:bg-transparent hover:bg-gray-200 dark:hover:bg-transparent flex items-center p-2 text-gray-700 font-normal rounded-lg dark:text-gray-400 dark:hover:text-white group">
...
export default {
    ...
    computed: {
        ...
        bonusPercent() {
            const value = this.setting && this.setting.initial_bonus;
