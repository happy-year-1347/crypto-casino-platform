                ...
                <div class="col-span-2 w-full">
                    ...
                    <div class="flex shadow dark:bg-gray-800 dark:border-gray-700 w-full rounded-lg">
                        ...
                        <div class="flex flex-col justify-center  p-4">
                            <h1 class="text-2xl font-bold mb-3">{{ $t('Request access') }}</h1>
                            <p class="text-gray-500">{{ $t('Our platform has an advanced affiliate panel where you can see your earnings and losses in detail and add sub-affiliates.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="commissionRewardsEl" tabindex="-1" aria-hidden="true" class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full overflow-y-auto overflow-x-hidden p-4 md:inset-0">
            <div class="relative max-h-full w-full max-w-2xl">
                ...
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    ...
                    <!-- Modal header -->
                    <div class="flex justify-between p-4 dark:bg-gray-600 rounded-t-lg">
                        <h1>{{ $t('Commission reward rules') }}</h1>
                    ...
                    <div class="flex flex-col  w-full justify-center p-4">
                            ...
                            <ul>
                                <li class="flex dark:bg-gray-800 shadow rounded-lg aposta-1 w-full p-4 mb-3">
                                    <div>
                                        ...
                                        <p class="text-gray-400 text-sm"><strong class="text-gray-400">{{ $t('Game') }}:</strong> {{ $t('Original games') }}</p>
                                    </div>
                                </li>
                                <li class="flex dark:bg-gray-800 shadow rounded-lg aposta-2 w-full p-4 mb-3">
                                    <div>
                                        <h1 class="font-mono text-4xl font-bold">7%</h1>
                                        <p class="text-gray-400 text-sm"><strong class="text-gray-400">{{ $t('Game') }}:</strong> {{ $t('Third-party slots, live casino') }}</p>
                                    </div>
                                </li>
                                <li class="flex dark:bg-gray-800 shadow rounded-lg aposta-3 w-full p-4 mb-3">
                                    <div>
                                        <h1 class="font-mono text-4xl font-bold">25%</h1>
                                        <p class="text-gray-400 text-sm"><strong class="text-gray-400">{{ $t('Game') }}:</strong> {{ $t('Sports') }}</p>
                                    </div>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div class="mt-5 w-full border dark:border-gray-500 p-4 rounded">
                            {{ $t('If you have any questions about our rules, please') }} <RouterLink to="/support" class="text-green-500 font-bold">{{ $t('contact us') }}</RouterLink>
                        </div>
                    ...
                    <div class="flex flex-col  w-full justify-center p-4">
                        <form action="" @submit.prevent="makeWithdrawal">
                            <div class="dark:text-gray-400 mb-3">
                                <label for="affiliate-amount">{{ $t('Withdrawal amount') }}</label>
                                <input id="affiliate-amount" v-model="withdrawalForm.amount" type="number" min="0.01" step="0.01" class="input" :placeholder="$t('Withdrawal amount')" required>
                                <span v-if="wallet" class="text-sm italic">{{ $t('Balance') }}: {{ state.currencyFormat(parseFloat(wallet?.refer_rewards), wallet?.currency) }}</span>
                            </div>

                            <div class="dark:text-gray-400 mb-3">
                                <label for="affiliate-coin">{{ $t('Select Cryptocurrency') }}</label>
                                <select id="affiliate-coin" v-model="withdrawalForm.pix_type" name="coin" class="input" required>
                                    <option value="">{{ $t('Select Cryptocurrency') }}</option>
                                    <option v-for="coin in cryptoCurrencies" :key="coin.code" :value="coin.code">{{ coin.label }}</option>
                                </select>
                            </div>

                            <div class="dark:text-gray-400 mb-3">
                                <label for="affiliate-address">{{ $t('Destination Address') }}</label>
                                <input id="affiliate-address" v-model="withdrawalForm.pix_key" type="text" class="input" :placeholder="$t('Enter your crypto address')" autocomplete="off" required>
                                <span class="text-xs">{{ $t('Make sure the address belongs to the selected coin and network') }}</span>
                            </div>

                            <button type="submit" class="mt-5 w-full bg-green-800 text-white hover:bg-green-600 px-4 py-2 transition duration-700">
                                {{ $t('Request now') }} <i class="fa-regular fa-arrow-right ml-2"></i>
                            </button>
                        </form>
                    </div>
                </div>
...
export default {
    ...
    methods: {
        ...
        getCryptoCurrencies: function() {
            const _this = this;
            HttpApi.get('crypto/currencies')
                .then(response => {
                    _this.cryptoCurrencies = response.data.currencies || [];
                })
                .catch(() => {
                    _this.cryptoCurrencies = [];
                });
        },
            ...
            HttpApi.post('profile/affiliates/request', _this.withdrawalForm)
                .then(response => {
                    ...
                    if(response.data.status === false) {
                        _toast.error(response.data.error);
                        return;
                    }
